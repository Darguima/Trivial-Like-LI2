...
void desenhaMenusLaterais(WINDOW *window, State *state)
{
	...
	{
		mvprintw(state->mapa.terminal.height - 4, state->mapa.terminal.width / 2 - 3, "%s", mobSobreposto->mob.nome);
		mvprintw(state->mapa.terminal.height - 2, state->mapa.terminal.width / 2 - 3, "%d", mobSobreposto->mob.vida);
	}
}